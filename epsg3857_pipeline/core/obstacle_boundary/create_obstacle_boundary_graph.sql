...
-- Create terrain grid points table if it doesn't exist
CREATE TABLE IF NOT EXISTS terrain_grid_points (
    id SERIAL PRIMARY KEY,
    geom geometry(POINT, :storage_srid)
);

-- If terrain_grid_points is empty, create a grid of points
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM terrain_grid_points) = 0 THEN
        -- Get the extent of water obstacles
        WITH extent AS (
            SELECT 
        ...
        INSERT INTO terrain_grid_points (geom)
        SELECT 
            ST_SetSRID(ST_MakePoint(x, y), :storage_srid) AS geom
        FROM 
            extent,
            generate_series(
                ST_XMin(bbox)::integer, 
                ST_XMax(bbox)::integer, 
                200
            ) AS x,
            generate_series(
                ST_YMin(bbox)::integer, 
                ST_YMax(bbox)::integer, 
                200
            ) AS y
        WHERE 
            -- Only include points that are not inside water obstacles
            NOT EXISTS (
                SELECT 1
                FROM water_obstacles wo
                WHERE ST_Contains(wo.geom, ST_SetSRID(ST_MakePoint(x, y), :storage_srid))
            );
    END IF;
END $$;

-- Create terrain edges table if it doesn't exist
...
-- If terrain_edges is empty, create edges between nearby points
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM terrain_edges) = 0 THEN
        INSERT INTO terrain_edges (source_id, target_id, length, cost, geom)
        SELECT 
            p1.id AS source_id,
            p2.id AS target_id,
            ST_Distance(p1.geom, p2.geom) AS length,
            ST_Distance(p1.geom, p2.geom) / 5.0 AS cost, -- Assuming 5 m/s speed
            ST_MakeLine(p1.geom, p2.geom) AS geom
        FROM 
            terrain_grid_points p1
        JOIN 
            terrain_grid_points p2
            ON p1.id < p2.id -- Avoid duplicate edges
            AND ST_DWithin(p1.geom, p2.geom, 300) -- Connect points within 300 meters
        WHERE 
            -- Ensure the edge doesn't cross through water obstacles
            NOT EXISTS (
                SELECT 1
                FROM water_obstacles wo
                WHERE ST_Crosses(ST_MakeLine(p1.geom, p2.geom), wo.geom)
            );
    END IF;
END $$;
