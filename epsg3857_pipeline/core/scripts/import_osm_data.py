...
# Configure logging
logger = get_logger('import_osm_data')
...
def install_tool(tool, container):
    ...
    run_command(
        f"/usr/bin/apt-get update",
        f"Update package lists",
        container
    )
    
    # Then install the tool
    run_command(
        f"/usr/bin/apt-get install -y {tool}",
        f"Install {tool}",
        container
    )
...
"""
    ...
    try:
        ...
        required_extensions = ["postgis", "hstore"]
        ...
        for extension in optional_extensions:
            try:
                ...
                logger.warning(f"⚠️ Optional extension {extension} could not be installed: {e}")
                logger.warning(f"Continuing without {extension} extension...")
