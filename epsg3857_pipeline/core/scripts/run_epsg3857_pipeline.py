...
# Configure logging
logger = get_logger('epsg3857_pipeline')
...
def run_water_obstacle_pipeline(mode="standard", config=None, standard=False, boundary=False):
    ...
    if mode == "standard":
        if boundary:
            cmd = f"python epsg3857_pipeline/alternatives/water_boundary/run_water_obstacle_pipeline_boundary.py"
        elif standard:
            cmd = f"python epsg3857_pipeline/alternatives/standard/run_water_obstacle_pipeline_crs.py"
        else:
            cmd = f"python epsg3857_pipeline/core/scripts/run_water_obstacle_pipeline_improved.py"
    elif mode == "delaunay":
        cmd = f"python epsg3857_pipeline/experimental/delaunay/run_water_obstacle_pipeline_delaunay.py"
    else:
    ...
    cmd += f" --sql-dir epsg3857_pipeline/core/sql"
...
def run_unified_delaunay_pipeline(threads=4, chunk_size=5000):
    ...
    cmd = f"python epsg3857_pipeline/experimental/delaunay/run_unified_delaunay_pipeline.py"
...
def import_osm_data(osm_file, container="db", db="gis", user="gis", verbose=False):
    ...
    cmd = f"python epsg3857_pipeline/core/scripts/import_osm_data.py"
...
def export_slice(lon, lat, minutes, outfile):
    ...
    cmd = f"python epsg3857_pipeline/core/scripts/export_slice.py"
...
def visualize_results(mode, input_file=None, show_unified=False):
    ...
    if mode == "graphml" and input_file:
        cmd = f"python epsg3857_pipeline/core/scripts/visualize.py --mode graphml --input {input_file}"
    elif mode == "water":
        cmd = f"python epsg3857_pipeline/core/scripts/visualize.py --mode water"
    elif mode == "delaunay":
        cmd = f"python epsg3857_pipeline/experimental/delaunay/visualize_delaunay_triangulation.py"
    elif mode == "obstacle-boundary":
        cmd = f"python epsg3857_pipeline/alternatives/obstacle_boundary/visualize_obstacle_boundary_graph.py"
        if show_unified:
            cmd += " --show-unified"
    else:
