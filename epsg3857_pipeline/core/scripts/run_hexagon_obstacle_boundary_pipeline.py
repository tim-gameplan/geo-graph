...
This script runs a simplified pipeline that:
...
# Set up logging
logger = setup_logger('hexagon_obstacle_boundary_pipeline')
...
def run_pipeline(config_file, sql_dir, container_name='geo-graph-db-1', verbose=False):
    ...
    # Run the hexagon obstacle boundary graph creation
    obstacle_boundary_sql = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'obstacle_boundary', 'create_hexagon_obstacle_boundary_graph.sql')
    if not run_sql_file(obstacle_boundary_sql, params, container_name, verbose):
        logger.error(f"Failed to run hexagon obstacle boundary graph creation")
        return False
...
def main():
    ...
    if success and args.visualize:
        ...
        visualize_cmd = f"python epsg3857_pipeline/core/scripts/visualize_hexagon_obstacle_boundary.py"
        if args.output:
