...
# Set up logging
logger = setup_logger('visualize_unified_boundary_graph_simple')
...
def fetch_data(connection):
    ...
    try:
        ...
        unified_boundary_nodes_gdf = gpd.GeoDataFrame(
            [{'id': row[0], 'element_type': row[1], 'element_subtype': row[2], 'geometry': wkb.loads(row[3])} for row in unified_boundary_nodes_data],
            geometry='geometry'
        )
        
        unified_boundary_edges_gdf = gpd.GeoDataFrame(
            [{'id': row[0], 'element_id': row[1], 'element_type': row[2], 'element_subtype': row[3], 'geometry': wkb.loads(row[4])} for row in unified_boundary_edges_data],
            geometry='geometry'
        )
