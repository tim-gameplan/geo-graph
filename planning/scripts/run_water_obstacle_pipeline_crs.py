#!/usr/bin/env python3
"""
CRS-standardized water obstacle modeling pipeline.
...
5. Only transforms to EPSG:4326 for export
"""
...
def run_pipeline(
    ...
    """
    Run the CRS-standardized water obstacle modeling pipeline.
    ...
    try:
        ...
        for sql_file in sql_files:
            ...
            if not os.path.exists(sql_path):
                ...
                logger.warning(f"Falling back to original version without _3857 suffix")
                
                # Try the original version without the _3857 suffix
                original_sql_file = sql_file.replace("_3857", "")
                original_sql_path = os.path.join(sql_dir, original_sql_file)
                
                if not os.path.exists(original_sql_path):
                    logger.error(f"Original SQL file not found: {original_sql_path}")
                    raise FileNotFoundError(f"SQL file not found: {sql_path} or {original_sql_path}")
                
                logger.info(f"Using original SQL file: {original_sql_file}")
                sql_path = original_sql_path
            
            execute_sql_file(conn, sql_path, params)
        
        logger.info("CRS-standardized water obstacle modeling pipeline completed successfully")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run the CRS-standardized water obstacle modeling pipeline")
    parser.add_argument(
        "--config",
