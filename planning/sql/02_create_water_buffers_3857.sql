...
-- :width_multiplier - Multiplier for width attribute
-- :min_width - Minimum buffer size for features with width attribute
...
SELECT
    ...
    -- Perform buffering directly in EPSG:3857 (meters)
    CASE
        -- For polygons
        WHEN feature_type = 'polygon' THEN
            CASE
                ...
                WHEN water_type = 'water' AND name ILIKE '%pond%' THEN 
                    ST_Buffer(geom, :buffer_pond)::geometry(MultiPolygon, 3857)
                WHEN water_type = 'reservoir' THEN 
                    ST_Buffer(geom, :buffer_reservoir)::geometry(MultiPolygon, 3857)
                ELSE 
                    ST_Buffer(geom, :buffer_default)::geometry(MultiPolygon, 3857)
            END
        -- For lines
        WHEN feature_type = 'line' THEN
            CASE
                ...
                WHEN water_type = 'river' THEN 
                    ST_Buffer(geom, :buffer_river)::geometry(MultiPolygon, 3857)
                WHEN water_type = 'stream' THEN 
                    ST_Buffer(geom, :buffer_stream)::geometry(MultiPolygon, 3857)
                WHEN water_type = 'canal' THEN 
                    ST_Buffer(geom, :buffer_canal)::geometry(MultiPolygon, 3857)
                WHEN water_type = 'drain' THEN 
                    ST_Buffer(geom, :buffer_drain)::geometry(MultiPolygon, 3857)
                WHEN water_type = 'ditch' THEN 
                    ST_Buffer(geom, :buffer_ditch)::geometry(MultiPolygon, 3857)
                ELSE 
                    ST_Buffer(geom, :buffer_default)::geometry(MultiPolygon, 3857)
            END
    END AS geom,
...
SELECT 
    buffer_rule_applied,
    COUNT(*) as count,
    MIN(buffer_size_m) as min_buffer_size_m,
    MAX(buffer_size_m) as max_buffer_size_m,
